...
- **Hermetic full suite.** `npm test` (alias `npm run test:full`) bootstraps isolated `HOME` / `P_DEV_HOME` / `TMPDIR` via `tests/setup/hermetic-env.ts`. Inherited `HARNESS_CONFIG_PATH` and live credentials are cleared. Do not depend on local `.env.local`, `.harness/config.local.json`, or `runs/` artifact caches. Use `npm run test:clean` for an explicit sanitized wrapper, and `npm run validate:checkpoint` for the Checkpoint gate.
