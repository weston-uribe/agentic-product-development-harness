import { describe, expect, it, vi } from "vitest";
import { createMacOsBrowserOpener } from "../../src/p-dev/browser.js";

describe("p-dev browser opener", () => {
  it("invokes macOS open with the target URL", async () => {
    const execFile = vi.fn(async () => ({ stdout: "", stderr: "" }));
    const opener = createMacOsBrowserOpener(execFile as never);
    const previousPlatform = process.platform;

    Object.defineProperty(process, "platform", { value: "darwin" });

    try {
      await opener.open("http://localhost:3000/settings/configure");
      expect(execFile).toHaveBeenCalledWith("open", [
        "http://localhost:3000/settings/configure",
      ]);
    } finally {
      Object.defineProperty(process, "platform", { value: previousPlatform });
    }
  });

  it("rejects non-macOS platforms in this spike", async () => {
    const execFile = vi.fn(async () => ({ stdout: "", stderr: "" }));
    const opener = createMacOsBrowserOpener(execFile as never);
    const previousPlatform = process.platform;

    Object.defineProperty(process, "platform", { value: "linux" });

    try {
      await expect(
        opener.open("http://localhost:3000/settings/configure"),
      ).rejects.toThrow("macOS-only");
      expect(execFile).not.toHaveBeenCalled();
    } finally {
      Object.defineProperty(process, "platform", { value: previousPlatform });
    }
  });
});
