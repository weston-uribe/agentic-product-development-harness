...
5. Trigger one captured product error without mutating GitHub, Linear, or Vercel:
   - `chmod -R a-w "$P_DEV_HOME/.harness"`
   - From Configure, attempt any observability preference write (for example **Reset local telemetry identity**).
   - Expected capture: `configure_request_error` / `configure_route` via `onRequestError` when `.harness` is read-only.
6. Stop the launcher cleanly so observability flush/shutdown hooks run.
...
8. Teardown: disable consent if needed, stop process, `unset P_DEV_SENTRY_DSN`, delete temp files.
