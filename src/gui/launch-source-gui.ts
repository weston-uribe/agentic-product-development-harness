...
async function waitForHealthyRootRoute(baseUrl: string): Promise<{
  ok: boolean;
  reason?: string;
  recoverableByCacheReset: boolean;
}> {
  await waitForConfigureServer(baseUrl, STARTUP_TIMEOUT_MS);
  const health = await checkGuiPageHealth(`${baseUrl.replace(/\/$/, "")}/`);
  return {
    ok: health.ok,
    reason: health.reason,
    recoverableByCacheReset: health.recoverableByCacheReset,
  };
}
...
async function runServerAttempt(input: {
  ...
  try {
    ...
    const health = await waitForHealthyRootRoute(baseUrl);
    if (!health.ok) {
      await cleanup();
      process.removeListener("SIGINT", onSignal);
      process.removeListener("SIGTERM", onSignal);
      const error = new Error(health.reason ?? "Harness GUI health check failed.");
...
export async function launchSourceGui(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (recoverable && attempt === 0) {
        ...
        console.log("Restarting PDev GUI once after cache cleanup…");
        continue;
      }
  ...
  throw new Error(
    cleanedCache
      ? "PDev GUI still failed the styling health check after cleaning apps/gui/.next."
      : "PDev GUI failed the styling health check.",
  );
}
